...
resource "azurerm_linux_virtual_machine_scale_set" "web_servers" {
  ...
  zones = [element(var.availability_zones, count.index % length(var.availability_zones))] # Specifies the availability zones for VM instances
}

////////////////////////////////////////////////////////
//// CREATE WEB VM AUTOSCALE SETTINGS ////
////////////////////////////////////////////////////////

resource "azurerm_monitor_autoscale_setting" "web_autoscale_settings" {
  name                = "web-autoscaling"                                      # Setting the name of the autoscale setting
  ...
  target_resource_id  = azurerm_linux_virtual_machine_scale_set.web_servers[0].id # Identifying the target resource for autoscaling
  ...
  profile {                 # Defining an autoscale profile
    ...
    rule { # Defining a scale-up rule
      metric_trigger {
        ...
        metric_resource_id = azurerm_linux_virtual_machine_scale_set.web_servers[0].id # Targeting the VMSS
        ...
        dimensions {
          ...
          values   = ["web1"]  # Dimension values to match (e.g., "web1")
        }
      }
    ...
    rule { # Defining a scale-down rule
      metric_trigger {
        metric_name        = "Percentage CPU" # Triggering on CPU usage percentage
        metric_resource_id = azurerm_linux_virtual_machine_scale_set.web_servers[0].id
        time_grain         = "PT1M"     # Time granularity for metric data (1 minute)
        statistic          = "Average"  # Using average metric data
        time_window        = "PT5M"     # Time window for the metric (5 minutes)
        time_aggregation   = "Average"  # Aggregating metric data using average
